...
/// 改造后的文法为：
/// primaryExp: T_DIGIT | T_L_PAREN expr T_R_PAREN | T_ID idTail
...
///
/// @brief 解析带有单目运算符的表达式。
/// 其文法为 unaryExp : T_SUB unaryExp | primaryExp
/// @return ast_node* 带有单目运算符或基本表达式的节点
///
static ast_node * unaryExp()
{
    if (match(T_SUB)) {
        ast_node * operand_node = unaryExp();

        if (!operand_node) {
            semerror("单目运算符 '-' 后缺少合法的表达式"); // Add a specific error message
            return nullptr;
        }
        return create_contain_node(ast_operator_type::AST_OP_MINUS, operand_node);
...
///
/// @brief 乘法、除法、求余运算符, 其文法为mulOp : T_MUL | T_DIV | T_MOD;
/// @return ast_operator_type
///
static ast_operator_type mulOp() // NEW HELPER FUNCTION
{
    ast_operator_type type = ast_operator_type::AST_OP_MAX;

    if (match(T_MUL)) {
        type = ast_operator_type::AST_OP_MUL;
        // match already advanced
    } else if (match(T_DIV)) {
...
///
/// @brief 乘法、除法、求余表达式，文法 mulExp : unaryExp (mulOp unaryExp)*
///
/// @return ast_node* 乘法、除法、求余表达式节点
///
static ast_node * mulExp()
{
    ...
    for (;;) {
        ...
        // Parse the right operand (another unaryExp)
        ast_node * right_node = unaryExp();
        if (!right_node) {
            semerror("二目运算符后缺少合法的表达式");
            break;
        }
